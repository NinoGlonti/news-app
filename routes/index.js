...
router.get("/articles", (req, res, next) => {
  const lang = [...req.user.languages].map(x => {
    return x.slice(0, 2)
  })
  let articlesFeed = [...req.user.category].map(x => getArticles(x, [...req.user.preferences], lang))
  Promise.all(articlesFeed)
    .then(feed => {
      const feedArticles = (feed.reduce((acc, val) => {
        return acc.concat(val.articles)
      }, []))
      res.render("articles", { user: req.user, chosenArticles: feedArticles.splice(0, 10) });
    }).catch(err => {
      console.log(err)
    })
})
